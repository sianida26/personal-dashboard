...
function normalizeEndpointPath(path: string): string {
	...
	const routePatterns = [
		...
		{ pattern: /^\/users\/[a-zA-Z0-9_-]+\/restore$/, template: '/users/:id/restore' },
		...
		{ pattern: /^\/observability\/requests\/[a-zA-Z0-9_-]+$/, template: '/observability/requests/:id' },
	];
...
const getEndpointOverviewEndpoint = createHonoRoute()
	...
	.get(
		...
		async (c) => {
			...
			if (startDate) {
				whereConditions.push(gte(observabilityEvents.timestamp, new Date(startDate)));
			}

			if (endDate) {
				whereConditions.push(lte(observabilityEvents.timestamp, new Date(endDate)));
			}
			...
			for (const record of rawData) {
				const normalizedEndpoint = normalizeEndpointPath(record.endpoint || '');
			...
			const endpointStats = Array.from(groupedStats.values()).map((group) => {
				const requests = group.requests;
				const totalRequests = requests.length;
				const responseTimes = requests.map(r => r.responseTimeMs || 0).filter(t => t > 0);
				const successCount = requests.filter(r => (r.statusCode || 0) < 400).length;
				const errorCount = requests.filter(r => (r.statusCode || 0) >= 400).length;
				
				// Calculate percentiles
				const sortedTimes = responseTimes.sort((a, b) => a - b);
				const p95Index = Math.ceil(sortedTimes.length * 0.95) - 1;
				const p95ResponseTime = sortedTimes.length > 0 ? sortedTimes[Math.max(0, p95Index)] : 0;
				
				const avgResponseTime = responseTimes.length > 0 ? 
					responseTimes.reduce((sum, time) => sum + time, 0) / responseTimes.length : 0;
				
				const lastRequest = requests.reduce((latest, current) => {
					const latestTime = latest.timestamp ? new Date(latest.timestamp) : new Date(0);
					const currentTime = current.timestamp ? new Date(current.timestamp) : new Date(0);
				...
				return {
					...
					p95ResponseTime: Math.round((p95ResponseTime || 0) * 100) / 100,
					successCount,
					errorCount,
					successRate: totalRequests > 0 ? 
						Math.round((successCount / totalRequests) * 100 * 100) / 100 : 0,
					errorRate: totalRequests > 0 ? 
						Math.round((errorCount / totalRequests) * 100 * 100) / 100 : 0,
					lastRequest,
				};
			});
