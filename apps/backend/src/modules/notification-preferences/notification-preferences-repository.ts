...
export const createNotificationPreferenceRepository = (
	...
	const ensureDefaultsForUser: NotificationPreferenceRepository["ensureDefaultsForUser"] = async (
		userId,
	) => {
		...
		const existingKeys = new Set(
			existing.map((record) => preferenceKey(record.category, record.channel)),
		);
	...
	const listPreferencesForUser: NotificationPreferenceRepository["listPreferencesForUser"] = async (
		userId,
	) => {
		return database
			.select()
	...
	const listOverrides: NotificationPreferenceRepository["listOverrides"] = async () => {
		return database.select().from(notificationChannelOverrides);
	};

	const upsertPreferences: NotificationPreferenceRepository["upsertPreferences"] = async (
		userId,
		payload,
	) => {
		if (!payload.length) {
		...
		const values = payload.map((item) => ({
			...
			source: (item.source ?? "user") as NotificationPreferenceSourceEnum,
			updatedAt: new Date(),
		}));
